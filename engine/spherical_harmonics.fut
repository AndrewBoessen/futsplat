...
def sh_to_color [L] ((x,y,z): (f32, f32, f32)) (dc_color: rgb) (coeffs: sh [L]) : rgb =
  ...
  let res_b1 =
    if deg >= 1 then
      let terms = [
        scale_rgb (0.0 - SH_L1 * y) coeffs[1],
        scale_rgb (      SH_L1 * z) coeffs[2],
        scale_rgb (0.0 - SH_L1 * x) coeffs[3]
      ]
  ...
  let res_b2 =
    if deg >= 2 then
      let terms = [
        scale_rgb (SH_L2[0] * xy) coeffs[4],
        scale_rgb (SH_L2[1] * yz) coeffs[5],
        scale_rgb (SH_L2[2] * (2.0 * zz - xx - yy)) coeffs[6],
        scale_rgb (SH_L2[3] * xz) coeffs[7],
        scale_rgb (SH_L2[4] * (xx - yy)) coeffs[8]
      ]
  ...
  let res_b3 =
    if deg >= 3 then
      let terms = [
        scale_rgb (SH_L3[0] * y * (3.0 * xx - yy)) coeffs[9],
        scale_rgb (SH_L3[1] * xy * z) coeffs[10],
        scale_rgb (SH_L3[2] * y * (4.0 * zz - xx - yy)) coeffs[11],
        scale_rgb (SH_L3[3] * z * (2.0 * zz - 3.0 * xx - 3.0 * yy)) coeffs[12],
        scale_rgb (SH_L3[4] * x * (4.0 * zz - xx - yy)) coeffs[13],
        scale_rgb (SH_L3[5] * z * (xx - yy)) coeffs[14],
        scale_rgb (SH_L3[6] * x * (xx - 3.0 * yy)) coeffs[15]
      ]
...
def precompute_color [n] [L]
  ...
  in map3 (\m c s -> (cam_view c_pos >-> sh_to_color) m c s) means3 rgbs shs
